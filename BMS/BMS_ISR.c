...
interrupt void ISR(void)
{
    ...
    Read_Adc_Data ();                                                               // Read ADC result
    ADC_Cali ();                                                                    // Calibration
...
void Feedback_Package_Prepare(void){
    ...
    if (Data_Package_Select == DC3_save_DC4_transmit && Data_Sample_Cycle_Count == Data_Sample_Cycle-1){
        Data_Center_3[Feedback_Package_Index + Data_Package_Size*0] = Slot_1.Volatge;
        Data_Center_3[Feedback_Package_Index + Data_Package_Size*1] = Slot_2.Volatge;
        Data_Center_3[Feedback_Package_Index + Data_Package_Size*2] = Slot_3.Volatge;
        Feedback_Package_Index++;
    } else if (Data_Package_Select == DC3_transmit_DC4_save && Data_Sample_Cycle_Count == Data_Sample_Cycle-1){
        Data_Center_4[Feedback_Package_Index + Data_Package_Size*0] = Slot_1.Volatge;
        Data_Center_4[Feedback_Package_Index + Data_Package_Size*1] = Slot_2.Volatge;
        Data_Center_4[Feedback_Package_Index + Data_Package_Size*2] = Slot_3.Volatge;
        Feedback_Package_Index++;
    }
...
void LED (void){
    if (SysState == ON) LED1_ON;
    else LED1_OFF;

    if (ChargeMode == Fast) LED2_ON;
    else LED2_OFF;
}
